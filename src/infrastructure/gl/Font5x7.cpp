...
const std::map<char, G>& table() {
    static const std::map<char, G> t = {
        {' ', {0,0,0,0,0,0,0}},
...
const G kBlank = {0, 0, 0, 0, 0, 0, 0};

} // namespace

const std::array<uint8_t, 7>& Font5x7::glyph(char c) {
    char up = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
    const auto& t = table();
    auto it = t.find(up);
    if (it == t.end()) return kBlank;
    return it->second;
}
